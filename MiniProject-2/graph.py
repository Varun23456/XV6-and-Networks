...
def plot_data_from_file(filename):
    ...
    filtered_data = [entry for entry in data if entry['ticks'] <= 500]
    ...
    plt.xlabel('Ticks')
    plt.ylabel('Queue')
    plt.title('Process Queue Over Time (First 200 Ticks,Boosttime 48 ticks)')
    plt.legend()
    plt.grid(True)
    
    # Set x-axis limit to 200 to ensure it extends up to 200
    plt.xlim(0, 200)
    
    # Save the plot to a file instead of showing it
    plt.savefig('fin7.png')


# Call the function with your file
plot_data_from_file('op.txt')
